...
// App-blocker IPC
ipcMain.on('appblock:add-workspace', async (_, { workspaceId, appName }) => {
  try {
    await blocker.add(appName);
    console.log(`Added blocked app: ${appName} for workspace ${workspaceId}`);
  } catch (err) {
    console.error('Error adding blocked app:', err);
  }
});

ipcMain.on('appblock:remove-workspace', async (_, { workspaceId, appName }) => {
  try {
    await blocker.remove(appName);
    console.log(`Removed blocked app: ${appName} for workspace ${workspaceId}`);
  } catch (err) {
...
app.on('will-quit', async () => {
  try { await blocker.clear(); } catch (err) {}
});
